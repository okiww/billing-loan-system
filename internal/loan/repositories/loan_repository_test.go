...
func TestGetLoanStatusByID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := tt.s.GetLoanStatusByID(context.Background(), tt.args.id)

			if (err != nil) != tt.wantErr {
				t.Errorf("GetLoanStatusByID() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if !tt.wantErr && !reflect.DeepEqual(got, tt.want) {
				t.Errorf("GetLoanStatusByID() got = %v, want %v", got, tt.want)
			}
